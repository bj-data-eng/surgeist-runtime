# surgeist-runtime Repository Guide
...
This independent leaf repository owns its manifest, runtime-domain
implementation, public front door, focused tests and docs, commits, and published
`main` candidate.

Root `surgeist` owns the facade and public composition, cross-crate adapters, root
integration tests and tools, this leaf's gitlink, and the API generator and
artifacts. A parent workspace, Codex project, task, branch, or worktree does not
change repository ownership.

## Discover The Current Structure

Read these sources instead of relying on a cached description.
...
| Focused verification | Tracked `#[cfg(test)]` modules, `tests/`, and fixtures when present |
| Additional configured commands | Cargo targets and features, plus `README.md`, a task runner, and CI when present |
| Integration MSRV, authoritative URL, and compatible pin | Root `Cargo.toml`, root `.gitmodules`, and the root's committed gitlink when root integration is in scope |
...
invalidation, frame scheduling, and provenance. It excludes domain parsers,
resolvers, algorithms, backends, and concrete task execution policy.
...
generator and all generated audit artifacts. This leaf repository carries no
copies.
