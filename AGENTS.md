...
Use this guide for automated work inside the Surgeist runtime crate repo.

## Crate Role

This repo owns `surgeist-runtime`, the app runtime orchestration contract crate
for Surgeist. Keep work inside this crate unless the user or root coordinator
explicitly directs otherwise.

Before starting, read the local `README.md`, active plan, and any referenced
guidance files. Treat root-authored plans as the source of runtime scope for
cross-crate work.
...
- Runtime owns orchestration contracts, not CSS parsing, style resolution
  internals, layout algorithms, text shaping, rendering backends, retained tree
  storage internals, template parsing, or window platform implementation.
